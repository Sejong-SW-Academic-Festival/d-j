...
function ScheduleDetails({ monthlySchedules, selDate, colors }) {
  ...
  let [heartSchedules, toggleHeartSchedules] = useState(
    new Array(toShowSchedules.length).fill(false)
  );

  toggleHeartSchedules = (event, index) => {
    console.log("클릭!!", index);
    heartSchedules[index] = !heartSchedules[index];
  };
        ...
        {toShowSchedules.map((e, index) => (
          <div key={e.id} className={"detail"}>
            ...
            <div
              ...
              onClick={(e) => toggleHeartSchedules(e, index)}
            >
              {heartSchedules[index] ? filledHeart : unFilledHeart}
            </div>
          </div>
        ))}
      </div>
